...
subtitle: "The Impact of Demographics and Economic Outcome in the 2020 Election"
author: 
  - Carl Fernandes
  - Lexi Knight
  - Raghav Bhatia
...
The analysis is based on the Cooperative Election Study (CES), from 2020 [@citeces2020_raw data] which contains a wealth of data on voter registration, preferences, demographics, and various opinions on national issues. The CES was conducted during a time of increased political activity and social discussions in the United States, providing a snapshot of the electorate during the 2020 Presidential election—a year marked by significant social and economic changes.
...
The model has 6 parameters  $\beta_0$, $\beta_1$, $\beta_2$, $\beta_3$, $\beta_4$, $\beta_5$ which need to be estimated from the [cleaned_data] dataset. These parameters are estimated by defining the model within the stan_glm() function from the [rstanarm] package. The function uses Markov Chain Monte Carlo (MCMC) algorithms to create a chain of samples from the posterior distribution of each coefficient. This not only gives us point estimates, but also the posterior distribution of values for each coefficient. 
...
The second column contains the actual parameter point estimates. These estimates are based of the stan_glm() function from the [@citerstanarm] package. The third column gives the standard error while the 4th and 5th columns give the lower and upper limit for the credibility intervals. The table was created from the model using the tidy() function from the [broom.mixed] package, while the 'kable()' and 'kable_styling()' functions were used from [@citeknitr] and [@citekable_extra]. 
